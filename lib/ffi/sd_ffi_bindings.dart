...
extension QuantizationTypeExtension on QuantizationType {
  String get displayName {
    switch (this) {
      case QuantizationType.f32: return 'FP32';
      case QuantizationType.f16: return 'FP16';
      case QuantizationType.q4_0: return 'Q4_0 (fastest)';
      case QuantizationType.q4_1: return 'Q4_1';
      case QuantizationType.q5_0: return 'Q5_0';
      case QuantizationType.q5_1: return 'Q5_1';
      case QuantizationType.q8_0: return 'Q8_0 (balanced)';
      case QuantizationType.q8_1: return 'Q8_1';
      case QuantizationType.q2_k: return 'Q2_K (smallest)';
      case QuantizationType.q3_k: return 'Q3_K';
      case QuantizationType.q4_k: return 'Q4_K (recommended)';
      case QuantizationType.q5_k: return 'Q5_K';
      case QuantizationType.q6_k: return 'Q6_K (near-lossless)';
  ...
  int get nativeValue {
    switch (this) {
      case QuantizationType.f32: return 0;
      case QuantizationType.f16: return 1;
      case QuantizationType.q4_0: return 2;
      case QuantizationType.q4_1: return 3;
      case QuantizationType.q5_0: return 6;
      case QuantizationType.q5_1: return 7;
      case QuantizationType.q8_0: return 8;
      case QuantizationType.q8_1: return 9;
      case QuantizationType.q2_k: return 10;
      case QuantizationType.q3_k: return 11;
      case QuantizationType.q4_k: return 12;
      case QuantizationType.q5_k: return 13;
      case QuantizationType.q6_k: return 14;
      case QuantizationType.q8_k: return 15;
    }
  }
}
...
extension BackendExtension on Backend {
  String get displayName {
    switch (this) {
      ...
      case Backend.vulkan: return 'Vulkan (GPU)';
      case Backend.opencl: return 'OpenCL (GPU)';
    }
  }

  String get libraryName {
    switch (this) {
      case Backend.cpu: return 'libsd_jni.so';
      case Backend.vulkan: return 'libsd_jni_vulkan.so';
      case Backend.opencl: return 'libsd_jni_opencl.so';
    }
  }
...
class SdFfiBindings {
    ...
    if (Platform.isAndroid) {
      ...
      try {
        ...
        if (backend != Backend.cpu) {
          print('[SdFfiBindings] Falling back to CPU backend');
          _lib = DynamicLibrary.open(Backend.cpu.libraryName);
          _currentBackendLib = Backend.cpu.libraryName;
        } else {
          rethrow;
        }
      }
  ...
  static void _lookupFunctions() {
    if (_lib == null) throw StateError('FFI library not loaded. Call initialize() first.');
    ...
    initEx = _lib!.lookupFunction<SdFfiInitExNative, SdFfiInitEx>('sd_ffi_init_ex');
    // Ensure the backend library is actually available; if not, mark it unavailable
    if (_currentBackendLib != Backend.cpu.libraryName) {
      final backend = Backend.values.firstWhere((b) => b.libraryName == _currentBackendLib);
